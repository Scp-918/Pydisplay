...
下位机控制区的初始值已经和固件 `1.2ms9444` 分支 `Core/Src/main.c` 里的 `g_sensor_param_array` 对齐：`k` 默认是 24；PPG mode 默认 `MultiLED`；Multi sub-mode 默认 `G-R-IR`；绿光/红光/IR 亮度默认是 `9/4/4`；PPG 量程默认 `4`；脉宽默认 `3`；陀螺仪量程默认 `500 dps`；加速度计量程默认 `2 g`。这样程序刚启动时，GUI 显示的控制参数和单片机初始化后正在使用的参数一致。
